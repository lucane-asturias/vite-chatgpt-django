<script lang="ts" setup>
  ...
  const promptInput = ref<HTMLInputElement>(null)
  const chatContainer = ref<HTMLDivElement>(null)

  const showChatList = ref<boolean>(false)
  const showDeleteButton = ref<boolean>(false)

  const toggleChatList = () => showChatList.value = !showChatList.value
...
<template>
  <ChatLayout>
    ...
    <template #aside>
      ...
      <ul :class="{ 'hidden': !showChatList }" class="sm:block">
        <router-link to="/chat">
          <li 
            class="
              flex justify-between font-semibold
              text-slate-100 bg-slate-900 hover:bg-slate-700 
              rounded-lg duration-200 px-4 py-2 my-2 mb-5
            "
          >
        ...
        <template v-for="content in chatStore.chats" :key="content.id">
          <router-link :to="`/chat/${content.id}`" tag="li"
            class="
              px-4 py-2 mb-4 flex justify-between font-semibold
              text-slate-400 hover:bg-slate-700 rounded-lg duration-200
            "
            :class="[ content.id === route.params.id ? 'bg-slate-700' : 'bg-slate-900' ]"
          >
            <span>{{ shortText(content.messages[0].message) }}</span>

            <div v-if="content.id === route.params.id">
              <button 
    ...
    <template #chat>
      <div class="flex flex-col-reverse sm:flex-row text-white">
        <div class="w-full flex h-screen">
          <div class="w-full overflow-y-auto" ref="chatContainer">

            <template v-if="!shouldRenderChildView">
    ...
    <!-- Chat Submit Form -->
    <template #form>
      <section class="fixed bottom-2 left-12 sm:left-0 right-0 p-5 sm:static w-5/6 sm:w-3/5 sm:p-0 sm:flex-none">
        <div class="relative flex-1 flex items-center mb-1">
          <input
            type="text"
            class="w-full h-16 bg-gray-800 text-white rounded-lg p-3 pl-5 pr-16 outline-none tracking-widest"
            placeholder="Send a message!"
            v-model.trim="messageInput"
            @keyup.enter="onMessageSubmition"
            ref="promptInput"
            :disabled="chatStore.chatInSubmission"
          />
          <!-- Send SVG -->
          <div class="absolute inset-y-0 right-0 flex items-center pl-3">
            <svg
              v-if="!chatStore.chatInSubmission"
              @click="onMessageSubmition"
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              stroke-width="1.5"
              stroke="currentColor"
              class="w-6 h-6 -ml-10 text-slate-200 cursor-pointer"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
            ...
            <div v-else class="dot-typing -ml-10" />
          </div>
        </div>
      </section>
    </template>
